...
/**
 * 번호 풍선 좌우 여백 — **글자 크기(em) 배수**. 풍선이 원을 벗어나 늘어나기 시작하는 기준이다.
 *
 * 풍선은 고정 반지름 원이었다(2026-08-30 검수 심각2). 그래서 `1F-01` 처럼 층 접두어가 붙은
 * 번호가 원 밖으로 삐져나갔다. 이제 글자 폭 + 이 여백이 지름보다 넓어지면 풍선이
 * **스타디움(알약)** 으로 늘어난다. 1~2자리 숫자는 여전히 넓어지지 않아 예전과 픽셀이 같다.
 */
export const BALLOON_TEXT_PAD_EM = 0.6;
 ...
 */
/**
 * @deprecated 2026-09-03 — 더 이상 쓰지 않는다.
 *
 * 촉 길이를 꺾은선 길이로 제한하던 상한이다. 안전진단 도면의 지시선은 짧은 경우가 많아
 * 이 상한에 자주 걸렸고, 사용자 눈에는 "길이에 따라 촉이 변한다" 로 보였다(실사용 신고).
 * 이제 촉은 도면 축척(`arrowHead` × zoom)에만 매인다. 상수는 이력을 위해 남긴다.
 */

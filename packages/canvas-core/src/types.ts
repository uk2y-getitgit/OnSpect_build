...
export type Memo = {
  ...
  /**
   * 앵커. 정규화 좌표.
   *   · 필기 메모 → 획 전체를 감싸는 상자의 **좌상단** (획과 함께 움직인다)
   *   · 옛 텍스트 메모 → 텍스트 상자의 **좌상단**
   */
  pos: NPoint;
  /**
   * 옛 텍스트 메모의 본문. **새 메모는 만들지 않는다** (F2 — 메모는 손글씨다).
   * 이미 저장된 레코드를 읽기 위해 남겨 둔다 — 지우면 옛 메모가 화면에서 사라진다.
   */
  text: string;
  /**
   * F2 — **필기(손글씨) 메모.** 사용자 요구: *"메모→자유그리기처럼 동작(필기메모)"*.
   *
   * 좌표는 절대 정규화 좌표다(`SketchPath` 와 같은 형식). `null` 또는 빈 배열이면
   * **옛 텍스트 메모**이고 노란 상자로 그린다 — 읽는 시점에 갈라 쓸 뿐,
   * IndexedDB 마이그레이션도 DB 버전 인상도 하지 않는다(`normalizeMemo` 참조).
   *
   * ⚠️ 결함의 `sketch` 와 **데이터도 표현도 다르다**: 메모는 결함에 속하지 않고,
   * 결함 상태색을 쓰지 않으며, 번호·리더선이 붙지 않는다.
   */
  paths: SketchPath[] | null;
  /** null = 기본 메모 스타일 상속 */
...
export type CanvasState = {
  ...
  /**
   * T-1 — **진행 중인 필기 세션**에서 방금 커밋한 메모 id. 없으면 null.
   *
   * 한 획 = 메모 하나라서 커밋 직후 그 메모가 자동 선택되고(§commitCreateMemoInk),
   * D14 규칙(선택 시 점선 상자)이 그대로 걸려 **글씨를 이어 쓰는 내내 상자가 붙어 있었다.**
   * 이 필드가 가리키는 메모는 상자를 그리지 않는다 — 세션이 끝나면(도구 변경 ·
   * 다른 곳 탭) null 로 돌아가고 D14 규칙이 되살아난다.
   *

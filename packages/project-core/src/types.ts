...
export type Drawing = RecordBase & {
   ...
   *
   * ⚠️ **2026-09-03 규칙 변경 — 이 값이 바뀌면 결함·메모 좌표도 함께 옮긴다.**
   * 예전에는 좌표를 안 건드렸는데(좌표가 A4 지면 기준 0~1 이라 안전해 보였다),
   * 실제로는 도면 그림만 커지고 표기는 제자리에 남아 **가리키던 자리에서 떨어져 나갔다**
   * (사용자 신고). 변환은 `a4Transform` 을 두 번 태운 것뿐이고
   * `[A4로 맞추기]`(`renormalizeAll`)와 **같은 기계**를 쓴다 — `apps/web/data/drawingScale.ts`.
   *

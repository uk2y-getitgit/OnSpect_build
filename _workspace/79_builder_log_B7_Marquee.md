...
| B7-d | ~~다중선택 상태에서 우측 패널은 예전 그대로(마지막 단일 선택 기준)다~~ → **정정(81_code-reviewer_findings 확인)**: 실제 코드는 마퀴 확정 시 `selection`을 `NO_SELECTION`으로 비운다. Inspector는 "선택된 표기가 없습니다"를 보여준다 — 마지막 단일선택을 남기면 그 결함만 편집 가능한 것처럼 보여 오히려 혼동을 준다는 판단. "N개 선택됨" 화면은 안 만들었다 | Inspector 분기 추가 |

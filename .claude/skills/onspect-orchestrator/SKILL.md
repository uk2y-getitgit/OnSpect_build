...
- 원인을 찾았는데 코드 결함이 아니라 **플랫폼/API 제약**으로 드러나면, builder 는 코드를
  건드리지 않고 멈춘다 — 좁은 수정이 아니라 범위 재협상이므로 그때부터는 질문 에스컬레이션
  절차(위)를 그대로 탄다
- builder 가 정적 확인으로 막혀 사용자에게 **그 자리에서** "브라우저로 확인해줘" 라는
  명시적 지시를 받으면, 이번 조사 한 건에 한해 `onspect-fix` 의 브라우저 예외 규칙이
  적용된다 (상세: `onspect-fix` §원인을 못 찾았을 때). "에이전트는 브라우저를 띄우지
  않는다"는 기본 원칙 자체는 바뀌지 않는다 — 사용자의 그 자리 승인이 있어야만, 그 건에
  한해서만 풀린다
...
### Phase 5-b — 배포 확인 (`main` 푸시로 반영되는 프로젝트)

이 프로젝트는 로컬 개발 서버 외에 **`main` 푸시 → Vercel 자동배포**로도 반영된다
(`NEXT.md` 배포 주소 참고). `main` 에 푸시하는 완료 절차를 탈 때는, **배포된 URL을 직접
폴링해서 반영 여부를 판단하지 않는다** — 빌드 성공/배포 생성/프로덕션 별칭 승격은 서로
다른 3단계이고, URL이 안 바뀐 것만으로는 어느 단계가 막혔는지 구분되지 않는다.

대신 푸시 직후 **커밋 자체의 배포 상태**를 먼저 확인한다:

```
GET https://api.github.com/repos/{owner}/{repo}/commits/{sha}/status
```

(공개 저장소면 인증 없이 호출 가능. Vercel 체크의 `state`/`description`과 배포 대시보드로
바로 가는 `target_url`이 온다.) 이 확인이 라이브 URL을 몇 분씩 폴링하는 것보다 먼저이고
더 정확하다 — "빌드는 됐는데 프로덕션 승격만 안 됐다"와 "빌드 자체가 안 됐다"를 URL
폴링만으로는 구분할 수 없다.

status API를 못 쓰거나(비공개 저장소·토큰 없음), status는 성공인데 라이브 URL이 아직
안 바뀌었으면 그 사실 그대로 보고한다 — `"빌드는 provider 기준 성공, 프로덕션 반영은
아직 확인 안 됨"` 처럼, 사용자가 어느 화면(배포 목록이지 빌드 로그가 아니다)을 봐야
하는지 알 수 있게 정확히 적는다. **"반영됐는지 확신할 수 없습니다" 같은 막연한 표현으로
뭉개지 않는다.**
